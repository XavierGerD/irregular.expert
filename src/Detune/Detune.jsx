...
export default class Detune extends Component {
	...
	handleOctaveSubdivisionChange = value => {
		...
		if (this.currentTemperament === "equal")
			this.getEqualTemperament(value);
	...
	handleGlobalDetune = value => {
		...
		pitchValues.forEach(pitchValue => {
			pitchValue.frequency =
				pitchValue.frequency * Math.pow(2, value / 1200);
		});
		...
		return (
			...
			>
				<div style={{ marginTop: "50px" }}>
					...
					>
						This app lets you play around with different
						temperaments. Click on the arrow to reveal a description
						of each tuning.
					</div>{" "}
					...
					<div className="detune-selector">
						...
						<div>
							...
							<div
								...
								style={{
									display: this.state.descriptions.equal
										? "block"
										: "none"
								}}
							>
								<MathJax.Provider>
									Equal temperament is obtained by making all
									half-steps (semitones) equal. In 12-tone
									equal temperament, the distance between all
									minor seconds is{" "}
									{
										<MathJax.Node
											inline
									...
									or{" "}
									{
										<MathJax.Node
											inline
											formula={"\\approx 1.059463"}
										/>
									}
									. While this allows all keys to be equally
									tempered, it has the tradeoff of preventing
									all intervals except the octave and unison
									from being perfectly just.
								</MathJax.Provider>
							</div>
						</div>
						<div>
							...
							<div
								className="description-box"
								style={{
									display: this.state.descriptions.just
										? "block"
										: "none"
								}}
							>
								<MathJax.Provider>
									Just intonation is a temperament in which
									interval sizes are determined using small
									whole number ratios, such as{" "}
									{<MathJax.Node inline formula={"3:2"} />}{" "}
									(perfect fifth) or{" "}
									{<MathJax.Node inline formula={"4:3"} />}{" "}
									(perfect fourth). These intervals are mostly
									derived from the harmonic series. While this
									temperament allows one specific key to have
									all of its intervals perfectly just, it
									sacrifices consonance in all other keys,
									since it is mathematically impossible to
									have just intervals in every key at the same
									time. When selecting just inonation, you may
									also choose the key to which the intervals
									are tuned.
								</MathJax.Provider>
							</div>
						</div>
						...
						<div>
							...
							<div
								className="description-box"
								style={{
									display: this.state.descriptions
										.quarterMeantone
										? "block"
										: "none"
								}}
